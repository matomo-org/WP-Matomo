...
* Security: the tracking code is now generated by Connect Matomo itself instead of being requested from Matomo through the SitesManager.getJavascriptTag API.
...
* Security: inside a WordPress network where Connect Matomo is activated site by site, the settings that decide which server the tracker is loaded from -- the Matomo URL, the cloud subdomain and the two CDN URLs -- may only name a host a network administrator allows. A new "Allowed tracker hosts" setting under Expert Settings holds that list, and only a network administrator can see or change it. When it is left empty the two Matomo cloud domains are allowed, so a network whose sites use a Matomo it runs itself has to name that host there. An entry written as a URL is read for the host it names, and an entry that names no host at all is reported rather than dropped silently. A network administrator may name any host. The "wp-piwik_allowed_tracker_hosts" filter can shape the list from code for a network that runs a Matomo per site.
* Security: the InnoCraft Cloud and Matomo Cloud subdomain settings are now checked to be a single host name label, so they can no longer be used to name a server outside the cloud they belong to. A subdomain that is not one is now reported instead of being dropped silently.
* Security: inside a WordPress network where Connect Matomo is activated site by site, changing the "Matomo connection method" is now checked against the allowed tracker hosts as well, so a site cannot start using a Matomo URL or cloud subdomain the network does not allow by switching to the connection method that uses it. Changing to a connection method that names the Matomo the site already uses is unaffected, and turning the connection off is never refused.
...
* Diagnostics: If your network was running an earlier version, a notice asks you to review two things once: the sites whose tracking code is entered manually, and the sites that load their tracker from a server the network does not allow. Both keep being used until you change them, whoever configured them, so please check that they are what you expect. Dismissing the notice removes it permanently: a site that used a disallowed tracker host before this release keeps it, and its administrator can still turn tracking back on with it, without anything reporting it again.
