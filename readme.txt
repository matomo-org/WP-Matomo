...
* Security: inside a WordPress network where Connect Matomo is activated site by site, the settings that decide which server the tracker is loaded from -- the Matomo URL, the cloud subdomain and the two CDN URLs -- may only name a host a network administrator allows. A new "Allowed tracker hosts" setting under Expert Settings holds that list, and only a network administrator can see or change it. When it is left empty the two Matomo cloud domains and the Matomo the network itself is connected to are allowed. A network administrator may name any host. The "wp-piwik_allowed_tracker_hosts" filter can shape the list from code for a network that runs a Matomo per site.
* Security: the InnoCraft Cloud and Matomo Cloud subdomain settings are now checked to be a single host name label, so they can no longer be used to name a server outside the cloud they belong to.
